...
class StripsFormParse:
    ...
    def dealWithPredicates(self):
        if self.c_section == 0: #initial
            ...
            if self.c_section % 2 == 0: # action name and preconds
                if self.lista_pred_names:
                    self.strips_action.name = self.lista_pred_names[0]
                else:
                    self.strips_action.name = ""
                self.strips_action.param = self.lista_ids_sep[0]
                list_aux = []

                for pred_name, pred_vars in zip(self.lista_pred_names[0:], self.lista_ids_sep[0:]):
        ...
        self.lista_pred_names = []
        self.cleanListIds()
    def dealWithAction(self, action_name):
        print(self.strips_actions)
        self.strips_actions[-1].name = action_name
        print(self.strips_actions)
    ...
    def __str__(self):
        return "\nInitial State: " + self.strips_initial + \
        "\n\tGoal State: " + str(self.strips_goal) + \
        "\n\tActions: " + str(self.strips_actions)
