...
class PDDLDomainParse:
    ...
    def dealWithFunctionDef(self):
        ...
        for i in range(0,len(self.lista_pddl_func)):
            ...
            if self.lista_pddl_func_types[i] not in self.domain_types and self.lista_pddl_func_types[i] != '(NOTYPE)':
                print("Semantic Error: type '{}' not defined in :types section".format(self.lista_pddl_func_types[i]))
                sys.exit()
            func.f_type = self.lista_pddl_func_types[i]
            for utype in self.lista_types:
                if utype not in self.domain_types and utype != '(NOTYPE)':
                    print("Semantic Error: type '{}' not defined in :types section".format(utype))
                    sys.exit()
    ...
    def appendConstants(self):
        ...
        if self.lista_pddl_ids:
            ...
            for curr_type, pddl_id in zip(self.dealing_with_types, self.lista_pddl_ids):
                if curr_type not in self.domain_types:
                    print("Semantic Error: type '{}' is not defined in :types section".format(curr_type))
                    sys.exit()
            ...
            print("Syntax Error: declaring constant type without a constant (missing constant declaration)")
            sys.exit()
    ...
    def dealWithAction(self):
        ...
        if (len(self.lista_types) != len(self.lista_pddl_vars[0]) ) and all(isinstance(n, list) for n in self.lista_pddl_vars[0]):
            print("Syntatic Error in action parameters variables after line %d: parameters must be all typed or all not typed"%(self.curLineAction))
