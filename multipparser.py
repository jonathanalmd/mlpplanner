...
class PDDLGroudedPredicate:
    def __init__(self, pred_name):
        self.name = pred_name
        self.p_gvars = [] # objs
    
    def __str__(self):
        return self.name + " " + str(self.p_gvars)
...
class mlpParser:
    ...
    def getGroundedgPredicates(self):
        preds = self.getPDDLDomainPredicates()
        grounded_preds = []
        for pred in preds:
            p_vars = pred.p_vars
            prob_objs = self.getPDDLProblemObjects() # dict {type:vars}
            untyped_objs = prob_objs["(NOTYPE)"]
            for subset in itertools.combinations(iter(untyped_objs), len(p_vars["(NOTYPE)"])):
                g_predicate = PDDLGroudedPredicate(pred.name)
                g_predicate.p_gvars = list(subset)
                grounded_preds.append(g_predicate)
        print(grounded_preds)
...
def p_lista_requirements_2(p):
    ...
    if p[2].upper() in ["STRIPS","TYPING","NEGATIVE-PRECONDITIONS","DISJUNCTIVE-PRECONDITIONS","EQUALITY","EXISTENTIAL-PRECONDITIONS","UNIVERSAL-PRECONDITIONS","QUANTIFIED-PRECONDITIONS","CONDITIONAL-EFFECTS","FLUENTS","NUMERIC-FLUENTS","ADL","DURATIVE-ACTIONS","DURATION-INEQUALITIES","CONTINUOUS-EFFECTS","DERIVED-PREDICATES","ACTION-COSTS"]:
        # colocar no lexer depois
        objDomain.domain_requirements.append(p[2].upper())
    else:
        print("Semantic Error: {} is not a valid requirement".format(p[2]))
        sys.exit()

def p_def_types_1(p):
    '''def_types : LPAREN COLON TYPES lista_types RPAREN'''
    objDomain.domain_types = objDomain.lista_types
...
def p_a_def_1(p):
    '''a_def : '''
    objDomain.curLineAction =  plex.lexer.lineno
...
def p_lista_parameters_2(p):
    '''lista_parameters : lista_avar MINUS ID lista_parameters'''
    # objDomain.cleanListaPDDLvars()
    if ("TYPING" in objDomain.domain_requirements) and (p[3] in objDomain.domain_types):
...
def p_lista_avar_2(p):
    '''lista_avar : VAR ID lista_var'''
